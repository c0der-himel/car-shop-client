...
const AllOrdersCard = ({ order, handleDeleteOrder }) => {
  const { _id, userName, userEmail, userPhoto, price, status, name, model } =
    order;
  ...
  const handleStatusUpdate = (id) => {
    axios
      .put(`https://powerful-garden-00570.herokuapp.com/updatestatus/${id}`, {
      ...
      <tr>
        ...
        <td className="px-6 py-4 whitespace-nowrap">
          <button
            ...
            className={
              status === 'shipped'
                ? 'px-4 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-300 text-green-700'
                : 'px-4 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-300 text-indigo-600'
            }
          >
            {updateStatus}
          </button>
